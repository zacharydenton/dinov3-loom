...
def main() -> None:
    ...
    emit("patch_w", weights["embeddings.patch_embeddings.weight"].reshape(R.HIDDEN, -1))
    ...
    for layer in range(R.LAYERS):
        ...
        emit(f"l{layer}_qkv_w", np.concatenate([
        ...
        emit(f"l{layer}_o_w", weights[p + "attention.o_proj.weight"])
        ...
        emit(f"l{layer}_gateup_w", np.concatenate([
        ...
        emit(f"l{layer}_down_w", weights[p + "mlp.down_proj.weight"])
